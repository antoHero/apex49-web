    ...
    <nav className="fixed top-0 right-0 left-0 z-[100] border-b border-[#191919]/10 bg-white/90 px-6 py-6 backdrop-blur-md lg:px-[56px]">
        ...
        <div className="flex flex-1 justify-end">
          <div className="hidden lg:block">
            <Link href="/request-quote">
              <button className="cursor-pointer border border-[#191919]/20 px-6 py-2 rounded-[6px] text-sm font-semibold text-[#191919] transition-all duration-300 hover:bg-[#191919] hover:text-white">
                Request a Quote
              </button>
            </Link>
          </div>
      ...
      {openNav && (
        <div className="absolute top-full left-0 flex w-full animate-in flex-col gap-8 border-b border-gray-200 bg-white p-8 shadow-2xl slide-in-from-top-2 lg:hidden">
          <NavList items={navItems} />
          <Link href="/request-quote" className="w-full cursor-pointer bg-[#191919] py-4 text-sm font-bold tracking-widest text-white uppercase">
            Request a Quote
          </Link>
        </div>
      )}
